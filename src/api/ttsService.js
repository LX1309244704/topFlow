...
const apiRequest = async (endpoint, data, method = 'POST', headers = {}) => {
  try {
    ...
    console.error('TTS API请求错误:', error);
    throw error;
  }
};
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i < retries - 1 && (error.message.includes('Failed to fetch') || error.message.includes('请求超时'))) {
        console.warn(`TTS API请求失败 (${i + 1}/${retries})，${error.message}，正在重试...`);
        // 指数退避策略
 ...
 * @param {string} model - TTS模型，默认为tts-1
 * @param {string} voice - 语音类型，默认为alloy
 * @param {string} responseFormat - 响应格式，默认为base64
 * @returns {Promise<string>} Base64编码的音频数据
 */
export const generateSpeech = async (text, model = 'tts-1', voice = 'alloy', responseFormat = 'base64') => {
  try {
    const response = await apiRequestWithRetry('/v1/audio/speech', {
      model: model,
      input: text,
      voice: voice,
